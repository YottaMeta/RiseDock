...
<p align="center">
  ...
  一个场景，一键开工 · 软件 / 文件 / 文件夹 / 网址收进场景 · 点一下按顺序启动该场景内已启用项<br>
  本地自动备份 · 可恢复
</p>
...
- **一键启动**：点场景即可启动该场景内所有已启用的启动项；可设启动间隔，避免同时挤爆系统
- **快捷启动**：设置里独立配置名称 / 路径 / 全局热键，一键打开单个应用、文件或网址（不占场景额度；与场景热键、显隐热键互斥检测）
- **系统组件**：菜单进入独立面板，单击执行本机电源与常用系统入口（关机 / 重启 / 锁屏、此电脑、控制面板等）；不进场景、不设热键
- **拖入添加**：场景详情内可从资源管理器直接拖入文件 / 文件夹（可多选）添加启动项；也支持「+ 添加」与批量添加
- 场景 / 全局 **快捷键**、列表拖拽排序、搜索跳转、导入导出与本地备份
- **16 款主题**：免费 6 款 + 会员 10 款（墨黑 / 霜蓝 / 玫瑰金 / 青瓷等）
