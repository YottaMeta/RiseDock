...
- **整理桌面**：扫桌面勾选进场景，默认移到「文档\启程典\桌面备份」，不默认删除
- **发现本机软件**：从开始菜单 / 桌面建场景，可加自定义扫描文件夹（便携盘 / Scoop 等），拒绝全盘
- **路径自愈**：文件找不到时，看板 / 热键 / 搜索 / 编辑器都会弹出定位
- **窗口位置记忆**：启动项可记录窗口位置，下次开工还原；可清除、可跳过单次
- **常用排序**：场景详情可按启动次数排「常用」
...
- 场景 / 全局 **快捷键**、列表拖拽排序、搜索（顶栏与 Ctrl+K 同一入口，回车仍启动）、按场景或全部导出、本地备份
- **16 款主题**：免费 6 款 + 会员 10 款（墨黑 / 霜蓝 / 玫瑰金 / 青瓷等）
- **授权门禁**：免费额度可用；过期冻结超额内容，续费后恢复（数据不删）；到期前会提醒，超限可直接去升级
